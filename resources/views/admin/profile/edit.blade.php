...
<div class="space-y-6">
    <!-- Page Header -->
    <div class="mb-8">
        <h1 class="text-4xl font-bold text-secondary-900">My Profile Settings</h1>
        <p class="mt-2 text-lg text-secondary-600">Manage your personal information and account settings</p>
    </div>
    ...
    <div class="overflow-hidden bg-white border shadow-sm rounded-xl border-slate-200">
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-200">
            ...
            <button onclick="toggleEditMode()" id="edit-btn"
                    class="inline-flex items-center px-4 py-2 text-sm font-medium text-white transition-colors rounded-lg bg-blue-600 hover:bg-blue-700">
        ...
        <!-- View Mode -->
        <div id="view-mode" class="p-6">
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
                <!-- Personal Information -->
                <div class="space-y-4">
                    <h4 class="text-sm font-semibold tracking-wide text-slate-700 uppercase">Personal Info</h4>
                    <div>
                        <label class="block text-xs font-medium text-slate-500">Full Name</label>
                        <p class="text-sm text-slate-900">{{ $user->first_name ?? $user->name }} {{ $user->middle_name ?? '' }} {{ $user->last_name ?? '' }}</p>
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-slate-500">Gender</label>
                        <p class="text-sm text-slate-900">{{ ucfirst($user->gender ?? 'Not specified') }}</p>
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-slate-500">Birthday</label>
                        <p class="text-sm text-slate-900">{{ $user->birthday ? $user->birthday->format('M d, Y') : 'Not specified' }}</p>
                    </div>
                </div>

                <!-- Contact Information -->
                <div class="space-y-4">
                    <h4 class="text-sm font-semibold tracking-wide text-slate-700 uppercase">Contact</h4>
                    <div>
                        <label class="block text-xs font-medium text-slate-500">Email</label>
                        <p class="text-sm text-slate-900">{{ $user->email }}</p>
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-slate-500">Contact Number</label>
                        <p class="text-sm text-slate-900">{{ $user->contact_number ?? 'Not specified' }}</p>
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-slate-500">Emergency Contact</label>
                        <p class="text-sm text-slate-900">{{ $user->emergency_contact ?? 'Not specified' }}</p>
                    </div>
                </div>

                <!-- Address Information -->
                <div class="space-y-4">
                    <h4 class="text-sm font-semibold tracking-wide text-slate-700 uppercase">Address</h4>
                    <div>
                        <label class="block text-xs font-medium text-slate-500">Street</label>
                        <p class="text-sm text-slate-900">{{ $user->street ?? 'Not specified' }}</p>
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-slate-500">Barangay</label>
                        <p class="text-sm text-slate-900">{{ $user->barangay ?? 'Not specified' }}</p>
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-slate-500">City/Municipality</label>
                        <p class="text-sm text-slate-900">{{ $user->city_municipality ?? 'Not specified' }}</p>
                    </div>
                    <div class="grid grid-cols-2 gap-2">
                        <div>
                            <label class="block text-xs font-medium text-slate-500">Province</label>
                            <p class="text-sm text-slate-900">{{ $user->province ?? 'N/A' }}</p>
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-slate-500">ZIP Code</label>
                            <p class="text-sm text-slate-900">{{ $user->zip_code ?? 'N/A' }}</p>
                        </div>
                    </div>
                    @if($user->id_photo)
                    <div class="pt-4 border-t border-slate-200">
                        <label class="block text-xs font-medium text-slate-500">ID Photo</label>
                        <div onclick="openIdPhotoModal()"
                             class="inline-flex items-center p-3 mt-2 transition-colors duration-200 bg-orange-50 border border-orange-200 rounded-lg cursor-pointer hover:bg-orange-100">
                            <svg class="w-5 h-5 mr-2 text-orange-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V8a2 2 0 00-2-2h-5m-4 0V5a2 2 0 114 0v1m-4 0a2 2 0 104 0m-5 8a2 2 0 100-4 2 2 0 000 4zm0 0c1.306 0 2.417.835 2.83 2M9 14a3.001 3.001 0 00-2.83 2M15 11h3m-3 4h2" />
                            </svg>
                            <span class="font-medium text-orange-800">View ID Photo</span>
                        </div>
                    </div>
                    @endif
                </div>
        ...
        <div id="edit-mode" class="hidden p-6 border-t border-slate-200">
            <form method="post" action="{{ route('admin.profile.update') }}" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
                    <div>
                        <label class="block mb-2 text-sm font-medium text-slate-700">First Name *</label>
                    ...
                    <div>
                        <label class="block mb-2 text-sm font-medium text-slate-700">Middle Name</label>
                        <input type="text" name="middle_name" value="{{ $user->middle_name }}"
                               class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    </div>
                    <div>
                        <label class="block mb-2 text-sm font-medium text-slate-700">Last Name *</label>
                ...
                <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
                    <div>
                        ...
                        <select name="gender" required class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    ...
                    <div>
                        <label class="block mb-2 text-sm font-medium text-slate-700">Birthday *</label>
                ...
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                    <div>
                        <label class="block mb-2 text-sm font-medium text-slate-700">Contact Number *</label>
                        <input type="tel" name="contact_number" value="{{ $user->contact_number }}" required placeholder="09123456789"
                               class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    </div>
                    <div>
                        <label class="block mb-2 text-sm font-medium text-slate-700">Emergency Contact *</label>
                        <input type="tel" name="emergency_contact" value="{{ $user->emergency_contact }}" required placeholder="09123456789"
                               class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    </div>
                </div>
                ...
                <!-- Address -->
                <div class="space-y-4">
                    <h4 class="text-base font-semibold text-slate-900">Address Information</h4>
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                        <div>
                            <label class="block mb-2 text-sm font-medium text-slate-700">Province *</label>
                            <select name="province" required class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                                <option value="Pangasinan" selected>Pangasinan</option>
                            </select>
                        </div>
                        <div>
                            <label class="block mb-2 text-sm font-medium text-slate-700">City/Municipality *</label>
                            <select name="city_municipality" required class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                                <option value="Alaminos City" selected>Alaminos City</option>
                            </select>
                        </div>
                        <div>
                            <label class="block mb-2 text-sm font-medium text-slate-700">Barangay *</label>
                            <select name="barangay" required class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                        ...
                        <div>
                            <label class="block mb-2 text-sm font-medium text-slate-700">Street *</label>
                            <input type="text" name="street" value="{{ $user->street }}" required
                                   class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                        </div>
                    </div>
                    <div class="md:w-1/2">
                        <label class="block mb-2 text-sm font-medium text-slate-700">ZIP Code *</label>
                        <select name="zip_code" required class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                ...
                <!-- ID Photo Upload -->
                <div>
                    <label class="block mb-2 text-sm font-medium text-slate-700">ID Photo</label>
                    <input type="file" name="id_photo" accept="image/*"
                           class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    <p class="mt-1 text-xs text-slate-500">Upload a clear photo of your government-issued ID (max 50MB)</p>
                    @if($user->id_photo)
                        <div class="mt-3">
                            <p class="mb-2 text-xs text-green-600">✓ Current ID photo:</p>
                            <img src="{{ asset('storage/' . $user->id_photo) }}" alt="Current ID Photo"
                                 class="h-auto max-w-xs border border-slate-300 rounded-lg">
                        </div>
                    @else
                        <p class="mt-1 text-xs text-slate-500">No ID photo uploaded yet.</p>
                    @endif
                </div>

                <div class="flex gap-3 pt-6">
                    <button type="submit"
                            class="px-6 py-2 font-medium text-white transition-colors rounded-lg bg-blue-600 hover:bg-blue-700">
                        Save Changes
                    </button>
                    <button type="button" onclick="toggleEditMode()"
                            class="px-6 py-2 font-medium text-white transition-colors bg-gray-600 hover:bg-gray-700 rounded-lg">
                        Cancel
                    </button>
                </div>
            </form>
    ...
    <div class="overflow-hidden bg-white border shadow-sm rounded-xl border-slate-200">
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-200">
            <div>
                ...
                <p class="mt-1 text-sm text-slate-600">Update your account password for security</p>
            </div>
            <button onclick="togglePasswordMode()" id="password-btn"
                    class="inline-flex items-center px-4 py-2 text-sm font-medium text-white transition-colors rounded-lg bg-red-600 hover:bg-red-700">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L15.232 5.232z"></path>
                </svg>
                Change Password
        ...
        <div id="password-mode" class="hidden p-6">
...
<div id="idPhotoModal"
    ...
    <div class="relative max-w-3xl overflow-hidden bg-white rounded-lg shadow-2xl">
        ...
        <div class="sticky top-0 z-10 flex items-center justify-between p-4 bg-white border-b border-slate-200">
            <h3 class="text-xl font-semibold text-slate-800">Your ID Photo</h3>
            <button onclick="document.getElementById('idPhotoModal').classList.add('hidden')"
                    class="p-2 text-slate-500 transition duration-150 rounded-full hover:bg-slate-100 hover:text-slate-700 focus:outline-none">
