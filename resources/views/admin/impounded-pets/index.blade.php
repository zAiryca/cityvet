...
@section('title', 'Impounded Pets Management')
...
<div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
    ...
    @php
        ...
        if (isset($currentStatus)) {
            if ($currentStatus === 'impounded') {
                ...
                $pageTitle = 'Denied Claims';
            } elseif ($currentStatus === 'requests') {
                $pageTitle = 'Pet Requests';
            }
        }
    @endphp
    ...
    <div class="p-6 bg-white shadow-md rounded-lg">
        ...
        <div class="flex justify-between items-center mb-6">
            ...
            <a href="{{ route('admin.impounded-pets.create') }}"
        ...
        <div class="bg-gray-50 p-4 rounded-lg mb-6">
            <form method="GET" action="{{ route('admin.impounded-pets.index') }}" class="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-6 gap-4">
                @if(isset($currentStatus))
                ...
                <div class="flex items-end space-x-2">
                    ...
                    <a href="{{ route('admin.impounded-pets.index', array_filter(['status' => $currentStatus ?? null])) }}" class="bg-gray-500 text-white px-4 py-2 rounded-md hover:bg-gray-600">Clear</a>
                </div>
            </form>
        </div>
        ...
        <div class="border-b border-gray-200 mb-6">
            <nav class="flex space-x-4 overflow-x-auto pb-2">
                @foreach($statuses as $label => $status)
                    <a href="{{ route('admin.impounded-pets.index', ['status' => $status]) }}"
                       class="@if($currentStatus === $status) border-indigo-500 text-indigo-600 @else border-transparent text-gray-500 hover:text-gray-700 @endif whitespace-nowrap py-2 px-3 border-b-2 font-medium text-sm transition duration-150 ease-in-out">
                       {{ $label }}
                    </a>
                @endforeach
            </nav>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse ($pets as $index => $pet)
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $pet->display_code }}</td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full
                                    ...
                                    @elseif($pet->status === 'claimed') bg-purple-100 text-purple-800
                                    @elseif($pet->registration_status === 'denied') bg-red-100 text-red-800
                                    @else bg-gray-100 text-gray-800 @endif">
                                    @if($pet->status === 'impounded') Impounded
                                    @elseif($pet->status === 'claimed') Claimed
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                ...
                                <a href="{{ route('admin.pets.edit', $pet) }}" class="text-green-600 hover:text-green-900 mr-3">Edit</a>
                                @if($pet->status === 'impounded')
                                    <form method="POST" action="{{ route('admin.pets.mark-claimed', $pet) }}" onsubmit="return confirm('Are you sure you want to mark this pet as claimed?')" style="display: inline;">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit" class="text-purple-600 hover:text-purple-900 mr-3">Mark as Claimed</button>
                                    </form>
                                @endif
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-4 text-center text-gray-500">No pets found for the status: **{{ $pageTitle }}**.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
